...
# 4. 서버 실행 확인 (포트는 환경 변수에 따라 변경)
curl http://localhost:8080/api/test/health  # 기본 포트
# 또는
curl http://localhost:${PORT}/api/test/health  # 환경 변수 사용 시
```
...
class ApiService {
  // 포트 번호는 백엔드 서버의 환경 변수 설정에 따라 변경
  // 기본값: User API 8080, Catalog API 8000
  
  static String get userApiBaseUrl {
    const port = 8080;  // 또는 환경에 따라 변경
    if (kIsWeb) return 'http://localhost:$port';
    else if (Platform.isAndroid) return 'http://10.0.2.2:$port';
    else if (Platform.isIOS) return 'http://localhost:$port';
    else return 'http://localhost:$port';
  }
  
  static String get catalogApiBaseUrl {
    const port = 8000;  // 또는 환경에 따라 변경
    if (kIsWeb) return 'http://localhost:$port';
    else if (Platform.isAndroid) return 'http://10.0.2.2:$port';
    else if (Platform.isIOS) return 'http://localhost:$port';
    else return 'http://localhost:$port';
...
**참고**: 백엔드 서버의 포트를 변경한 경우, Flutter 코드의 포트 번호도 함께 변경해야 합니다.
...
**로그 예시**:
```
2025-11-08 10:00:00.000 DEBUG [http-nio-8080-exec-1] c.c.u.s.JwtTokenProvider : JWT 토큰 생성: userId=1
2025-11-08 10:00:01.000  INFO [http-nio-8080-exec-2] c.c.u.c.AuthController : 개발용 로그인 성공: email=test@example.com
```
...
cd be/catalog-api && source .venv/bin/activate && python main.py
...
```bash
# User API: Ctrl+C
# Catalog API: Ctrl+C (가상환경에서 실행 중인 프로세스 종료)
# Flutter: Ctrl+C
```
...
```bash
# 1. 서버에 코드 업로드
scp -r be/catalog-api ec2-user@server:/app/
...
# 5. 서버 실행 (백그라운드)
nohup python main.py > catalog-api.log 2>&1 &
```
...
#### 문제: API 연결 실패
```
SocketException: Failed to connect to localhost:8080
```

**해결**:
1. User API와 Catalog API가 실행 중인지 확인
2. 포트 번호 확인 (8080, 8000)
3. 플랫폼별 URL 확인:
   - 웹: localhost
   - Android: 10.0.2.2
   - iOS: localhost
