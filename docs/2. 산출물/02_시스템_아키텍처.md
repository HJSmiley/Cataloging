...
│  - /api/items/** → Catalog API                         │
│  - /api/user-catalogs/** → Catalog API                 │
│  - /api/upload/** → Catalog API                        │
│                                                         │
│  포트: 환경 변수로 관리 (기본 User:8080, Catalog:8000) │
└─────────────────────────────────────────────────────────┘
            │                           │
            ▼                           ▼
┌──────────────────────┐    ┌──────────────────────────┐
│   User API (8080)    │    │  Catalog API (8000)      │
│   Spring Boot 3.2.0  │    │  FastAPI + Python venv   │
│                      │    │                          │
│  - JWT 발급          │    │  - JWT 검증              │
│  - 회원 관리         │    │  - 카탈로그 CRUD         │
│  - OAuth2 로그인     │    │  - 아이템 CRUD           │
...
카탈로깅 시스템에서 User API와 Catalog API는 기본적으로 독립적으로 동작하지만, 특정 상황에서 서로 통신합니다.

#### 7.1.1 기본 통신 방식 (JWT 기반)
...
#### 7.1.2 공개 카탈로그 조회 시 통신

공개 카탈로그 목록에서 생성자 닉네임을 표시하기 위해 Catalog API가 User API를 호출합니다.
...
[Catalog API]
    │
    │ 1. 공개 카탈로그 조회 (SQLite)
    │    SELECT * FROM catalogs WHERE visibility='public'
    │
    │ 2. 각 카탈로그의 생성자 정보 조회
    │    ┌─────────────────────────────────────┐
    │    │ HTTP GET 요청                        │
...
**주요 특징**:
- User API의 `/api/users/{userId}` 엔드포인트는 인증 불필요 (공개 프로필)
- Catalog API는 Python `requests` 라이브러리 사용
- 닉네임 캐싱으로 중복 호출 최소화
- 실패 시 "알 수 없음"으로 표시 (서비스 독립성 유지)

### 7.2 의존성 관리

| 항목 | User API | Catalog API |
|------|----------|-------------|
| **JWT 시크릿** | 발급 시 사용 | 검증 시 사용 |
| **사용자 정보** | 저장 및 관리 | user_id만 저장 |
| **닉네임 조회** | 제공 (공개 API) | 필요 시 호출 |
| **독립성** | 완전 독립 | User API 선택적 의존 |

### 7.3 통신 실패 처리

```python
# Catalog API에서 User API 호출 시
try:
    response = requests.get(f"{user_api_url}/{user_id}", timeout=2)
    if response.status_code == 200:
        nickname = response.json().get("nickname", "알 수 없음")
    else:
        nickname = "알 수 없음"
...
**장점**:
- User API 장애 시에도 Catalog API는 정상 동작
- 닉네임만 표시되지 않을 뿐, 카탈로그 조회는 성공
- 서비스 간 느슨한 결합 유지
...
- **가상환경 격리**: Python venv로 의존성 관리 및 환경 일관성
